<template>
  <div>
    ...
    <Modal
      ...
      v-model="visibleErrorAns"
      footer-hide
      :width="795"
      :scrollable="true"
      @on-visible-change="modalQueVisible"
      class="iview-modal">
    <upload-img v-if="typeShow == 'dy'" :getQuestion="getQuestion" @modalShowUpdate="modalShowUpdate"></upload-img>
    <error-correction v-if="typeShow == 'jc'" :getQuestion="getQuestion" @modalShowUpdate="modalShowUpdate"></error-correction>
  </Modal>
</div>
</template>
...
export default {
  props: ['topics', 'getQuestion', 'total'],
  // props: {
  //   topics: {
  //     type: Array
  //   },
  ...
  methods: {
    ...
    modalShow ({ ID }, type) {
      this.typeShow = type
      this.getQuestion.question_id = ID
      this.visibleErrorAns = true
      // this.$emit('modalShow', true, ID, type)
    },
    modalQueVisible (val) {
      document.body.removeAttribute('style')
      if (!val) {
        this.visibleErrorAns = false
        this.typeShow = ''
      }
    },
    modalShowUpdate () {
