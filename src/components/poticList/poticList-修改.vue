<template>
  ...
  <div class="topic-main" v-if="getQuestion.jiexi == 1">
    <div class="topic-list" :class="{'topic-list-pb': item.flag}" :id="'anchor-' + index" v-for="(item, index) in listData" :key="index">
      <div class="topic-top">
        <span class="topic-num"><em>{{index+1}}</em>/{{listData.length}}</span>
...
export default {
  ...
  data () {
    return {
      listData: JSON.parse(JSON.stringify(this.topics))
    }
  },
  computed: {
    ...mapState({
  ...
  methods: {
    ...
    //   this.listData[index].flag = !currentFlag
    //   // this.$forceUpdate()
    // },
    resolveToggle: lodash.throttle(function (currentFlag, index) {
      this.listData[index].flag = !currentFlag
    }, 300),
    // 收藏
    qtCollection (item) {
