        ...
        <div v-if="newsFlag">
          <ul class="news-list">
            <li class="news-item" :class="{'invalid': item.status == 2}" v-for="(item, index) in systeMessageList" :key="index" @click="getRead(item)">
              ...
              <!-- 1课程 -->
              <div class="news-center" v-if="item.type == 1">
                <h3>您的答疑有新的回复<em>{{item.create_time}}</em></h3>
                <p>您的提问{{item.title}}有新的回答</p>
              </div>
              <!-- 2题库 -->
              <div class="news-center" v-if="item.type == 2">
                <h3>您的答疑有新的回复<em>{{item.create_time}}</em></h3>
                <p>您的提问{{item.title}}有新的回答</p>
              </div>
              <!-- 3订单 -->
              ...
              <!-- 4普通 5链接 -->
              <div class="news-center" v-if="item.type == 4 || item.type == 5">
                <h3><span>{{item.title}}</span><em>{{item.create_time}}</em></h3>
                <p>{{item.content}}</p>
              </div>
              <!-- 6直播 -->
              <div class="news-center" v-if="item.type == 6">
                <h3><span>{{item.title}}</span><em>{{item.create_time}}</em></h3>
                <p>{{item.content}}</p>
              </div>
            </li>
