    ...
    <el-row class="tac">
      <el-col :span="24">
        <el-menu
          ...
          <el-submenu :index="String(item.index)" v-for="item in courseCatalogInfo" :key="item.course_id" style="margin-bottom: 10px;border:0;">
            <template slot="title" >
              <div class="title-cpe" @click="getSecvCatalog(item, index)">
                <span><i class="elt-icon elt-icon-01"></i>{{item.name}}{{item.index}}</span>
                <span>{{item.cpe_integral}}积分</span>
              </div>
            </template>
            <el-submenu :index="String(item.index+'-'+val.index)" v-for="val in secvCatalogList[item.course_id]" :key="val.section_id">
              <template slot="title">
                <i class="elt-icon elt-icon-02"></i>
                <span>{{val.section_name}}{{item.index+'-'+val.index}}</span>
              </template>
              <el-menu-item :index="String(item.index + '-' + val.index + '-' + v.index)" v-for="(v) in val.video" :key="v.video_id"
              @click="playVideo(item, val, v, key, index)">
                <i class="elt-icon elt-icon-stop"></i>
                <span>{{v.video_name}}{{item.index + '-' + val.index + '-' + v.index}}</span>
              </el-menu-item>
            </el-submenu>
          </el-submenu>
        </el-menu>
