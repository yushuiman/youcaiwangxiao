      ...
      <el-col :span="24">
        <el-menu class="el-menu-vertical-demo">
          <el-submenu :index="item.index+''" v-for="item in courseCatalogInfo" :key="item.course_id" style="margin-bottom: 10px;border:0;">
            ...
            {{secvCatalogList}}
            <!-- {{val}} -->
            <el-submenu :index="item.index+'-'+val.index+''" v-for="val in secvCatalogList" :key="val.section_id">
  ...
  methods: {
    ...
    getSecvCatalog (item, index) {
      ...
      secvCatalog({
        ...
        const res = data.data
        if (res.code === 200) {
          this.secvCatalogList = res.data
          this.secvCatalogList.forEach((v, index) => {
            v.index = index + 1 + ''
